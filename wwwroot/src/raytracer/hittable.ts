...
export class Sphere implements IHittable {
    ...
    hit(ray: Ray, tMin: number, tMax: number): HitRecord | null {
        ...
        const [normal, frontFace] = correctNormal(ray, scale(subtract(point, this.#centre), 1 / this.#radius));

        return { point, normal, t, frontFace };
    }
}

function correctNormal(ray: Ray, normal: Vec3): [correctedNormal: Vec3, frontFace: boolean] {
